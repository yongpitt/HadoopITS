...
public class HdfsFileObject implements FileObject {
  ...
  public HdfsFileObject(String path, User user) {	  
    if(path.startsWith("hdfs://")){
      ...
      if(path.startsWith(Path.SEPARATOR)){
        this.path = new Path(user.getHomeDirectory() + path.substring(Path.SEPARATOR.length()));
      }else{
  ...
  public String getFullName() {
	String fullName;
	if(user.getHomeDirectory().endsWith(Path.SEPARATOR)){
	  fullName = path.toString().substring(user.getHomeDirectory().length() - 1);
	}else{		
	  fullName = path.toString().substring(user.getHomeDirectory().length());	  
	}
	if(fullName == null ||fullName.equals(""))fullName = Path.SEPARATOR;
	return fullName;
  }
  ...
  public String getShortName() {
    ...
    int pos = full.lastIndexOf(Path.SEPARATOR);
    if (pos == 0) {
...
//        }
  ...
  private HdfsFileObject getParent() {
    ...
    String parentS = Path.SEPARATOR;
    int pos = pathS.lastIndexOf(Path.SEPARATOR);
    if (pos > 0) {
      parentS = pathS.substring(0, pos + 1);
    }
    if(parentS.equals(user.getHomeDirectory())){
      parentS = Path.SEPARATOR;
    }else{
      parentS = parentS.substring(user.getHomeDirectory().length());    	
    }
  ...
  public boolean hasWritePermission() {
	...
	if(user.isWritePermission() == false)return false;
